...
// ---------- AKILLI RASTGELELİK (Bag sistemi) ----------
let pieceBag = [];
let colorBag = [];
...
function getRandomPiece() {
  if (pieceBag.length === 0) pieceBag = shuffle(BASE_PIECES);
...
function getRandomColor() {
  if (colorBag.length === 0) colorBag = shuffle(COLORS);
  return colorBag.pop();
}
// -----------------------------------------------------

// 🔄 Matris döndürme (parçalar için)
...
function placePiece(x0, y0) {
  ...
  // Uygun yere sığıyor mu kontrolü
